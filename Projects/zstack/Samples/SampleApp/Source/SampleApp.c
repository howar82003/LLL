...
extern unsigned char ReadData(unsigned int i);
...
uint8 MYTASK_TaskID;      //我的任務的全域變數
uint8 TakeValue_TaskID;
uint8 Operation_TaskID;
uint8 SendUART_TaskID;

unsigned char messageA = 0x30 ;
unsigned char periodicData[28];
unsigned char SEND_DATA[28];
unsigned char CNT[3]={0x30,0x30,0x30};

unsigned int tempA = 0 ;
unsigned int cntnumberA= 0 ;
unsigned int missmessageA = 0 ;

unsigned int tempB = 0 ;
unsigned int cntnumberB= 0 ;
unsigned int missmessageB = 0 ;

unsigned char MMchar[33] = "#####_Dropped Packets_=____ #####";
unsigned char ROOM1[16] ="*****ROOM A*****";
unsigned char ROOM2[16] ="*****ROOM B*****";
...
{ 
  ...
  MT_UartInit();                  //串口初始化
  MT_UartRegisterTaskID(task_id); //註冊串口任務
...
/*************************************************************/
/**********                            SendUART                        ************/
/**********                   When Coordinator, do this           ************/
void SendUARTInit(uint8 task_id)
{
  SendUART_TaskID = task_id;
  Init_Sensor();
}
...
/*************************************************************/
/**********                            Take Value                         ************/
/**********           when EndDevice , do this task.             ************/
/**********              BackGround & ReadSensor               ************/
void TakeValueInit(uint8 task_id)
{
    TakeValue_TaskID = task_id;
    
}

uint16 TakeValue(uint8 taskid, uint16 events)
{
   if ( events & TAKE_VALUE)
   {    
      int i , k ;  
      for( i=0 ; i<5; i++) 
      {
          // HalLedBlink( HAL_LED_3, 1, 20, 40 );
          SensorFunction();
       //   asm("nop");asm("nop");asm("nop");asm("nop");asm("nop");
      }
      for(k=0;k<24;k++)
      {
          SEND_DATA[k]=ReadData(k);
      }   return (events ^TAKE_VALUE) ; 
   }  return 0;
}
/*=====================================================*/
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    while ( MSGpkt )
    {
      ...
      {        
        ...
        case AF_INCOMING_MSG_CMD:
          ...
          osal_set_event( SendUART_TaskID, SEND_UART );
  ...
  { 
    osal_set_event( TakeValue_TaskID, TAKE_VALUE );
 ...
 *
 * @brief   Send the P2P message to Coordinator.
 *
 * @param   Use CNT to count miss message.
 *
 * @return  none
 */
void SampleApp_Send_P2P_Message( void )
{
  LedState = LedState+1 ;
  //unsigned char data[15]= "F3332221114444";
  SEND_DATA[0] = 0x41;         // ROOMA SESOR KEY
  ...
  //SEND_DATA[14]=SEND_DATA[0];         //ROOM1  CNT  NUMBER   (A or B)
  SEND_DATA[24]=CNT[0];
  SEND_DATA[25]=CNT[1];
  SEND_DATA[26]=CNT[2];
